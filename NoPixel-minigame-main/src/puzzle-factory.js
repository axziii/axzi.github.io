...
export function generateQuestionAndAnswer(nums, puzzles, shownNums){

    // For this version we need to randomly grab a section of the numbers which have been chosen.
    
    const randomNumberOne = shownNums[Math.floor(Math.random()*(shownNums.length))];
    let randomNumberTwo
    do {randomNumberTwo = shownNums[Math.floor(Math.random()*(shownNums.length))]} while (randomNumberOne === randomNumberTwo);
    
    // Find the index of the number in the main list for use with the questions
    const positionOne = nums.indexOf(randomNumberOne);
    const positionTwo = nums.indexOf(randomNumberTwo);
